...
Observação qualitativa realizada após a ativação de `spring.threads.virtual.enabled=true` (Prompt A / correção do Prompt 3.1) e implementação do bean `VirtualThreadConfig` (Prompt 3.2):

- **Confirmação de Virtual Threads a servir pedidos HTTP:** Um log DEBUG temporário em `IssueController.findAll` registou `VirtualThread[#80,tomcat-handler-8]/runnable@ForkJoinPool-1-worker-3` durante um pedido GET autenticado a `/api/v1/issues`, confirmando que os pedidos são servidos por Virtual Threads.
- **Impacto no número de platform threads:** Antes de uma rajada de 20 pedidos concorrentes, `jvm.threads.live` marcava 28 threads. Após a rajada, o valor subiu para 32 — um aumento de apenas 4 threads de plataforma, demonstrando que as Virtual Threads não escalam o número de platform threads proporcionalmente ao número de pedidos concorrentes.

**Nota:** Esta verificação foi repetida em 2026-07-29 como parte da correção documentada em `Report.md` (Prompt C), uma vez que a observação original foi registada antes da ativação efetiva de `spring.threads.virtual.enabled=true` no repositório. É exploratória e qualitativa, não substituindo a validação formal de desempenho (RNF-01) reservada para a Fase 8 (JMH/k6). Nenhum dado aqui apresentado deve ser confundido com a métrica de "60% de redução de latência" — essa alegação só pode ser sustentada pelos testes reprodutíveis da Fase 8.
